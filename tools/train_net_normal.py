...
from cluster import get_center
...
def train(cfg):
    ...
    # Create meters.
    if cfg.DETECTION.ENABLE:
        train_meter = AVAMeter(len(train_loader), cfg, mode="train_label")
        val_meter = AVAMeter(len(val_loader), cfg, mode="val_dry")
    else:
        train_meter = TrainMeter(len(train_loader), cfg)
        val_meter = ValMeter(len(val_loader), cfg)
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if is_eval_epoch:
            ...
            _ = eval_epoch(val_dark_loader, model, val_dark_meter, loss_scaler, cur_epoch, cfg, writer)
            if flag:
