...
def multiple_samples_collate_train_pseudo_label(batch, fold=False):
    """
    Collate function for repeated augmentation. Each instance in the batch has
    more than one sample.
    Args:
    ...
    Returns:
        (tuple): collated data batch.
    """
    inputs, labels, video_idx,probs, extra_data = zip(*batch)
    # print(inputs, flush=True)
    # print(labels, flush=True)
    # print(video_idx, flush=True)
    inputs = [item for sublist in inputs for item in sublist]
    ...
    video_idx = [item for sublist in video_idx for item in sublist]
    inputs, labels, video_idx,probs, extra_data = (
        default_collate(inputs),
        default_collate(labels),
        default_collate(video_idx),
        default_collate(probs),
        default_collate(extra_data),
    )
    # print("probs",type(probs),probs.size(),probs)
    if fold:
        return [inputs], labels, video_idx,probs, extra_data
    else:
        return inputs, labels, video_idx,probs, extra_data
...
def construct_loader(cfg, split, is_precise_bn=False, decording=None):
    """
    ...
    dataset = build_dataset(dataset_name, cfg, split, decording)
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        ...
        if (
            ...
            and split in ["train_label", "train_unlabel", "train_pseudo_label"]
            ...
            elif cfg.AUG.NUM_SAMPLE > 1 and split in ["train_label", "train_unlabel"]:
                collate_func = partial(
                    multiple_samples_collate, fold="imagenet" in dataset_name
                )
            elif cfg.AUG.NUM_SAMPLE > 1 and split in ["train_pseudo_label"]:
                collate_func = partial(
                    multiple_samples_collate_train_pseudo_label, fold="imagenet" in dataset_name
                )
            else:
                collate_func = None
