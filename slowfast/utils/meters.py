...
class TrainMeterPseudo(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.mb_top1_err = ScalarMeter(cfg.LOG_PERIOD)
        self.mb_top5_err = ScalarMeter(cfg.LOG_PERIOD)
        self.mb_target_top1_err = ScalarMeter(cfg.LOG_PERIOD)
        self.mb_target_top5_err = ScalarMeter(cfg.LOG_PERIOD)
        # Number of misclassified examples.
        self.num_top1_mis = 0
        self.num_top5_mis = 0
        self.num_target_top1_mis = 0
        self.num_target_top5_mis = 0
        self.num_samples = 0
        self.output_dir = cfg.OUTPUT_DIR
    ...
    def reset(self):
        ...
        self.mb_top1_err.reset()
        self.mb_top5_err.reset()
        self.mb_target_top1_err.reset()
        self.mb_target_top5_err.reset()
        self.num_top1_mis = 0
        self.num_top5_mis = 0
        self.num_samples = 0
    ...
    def update_stats(self, top1_err, top5_err, target_top1_err, target_top5_err, loss, lr, mb_size):
        """
        Update the current stats.
        Args:
            top1_err (float): top1 error rate.
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            ...
            self.mb_top1_err.add_value(top1_err)
            self.mb_top5_err.add_value(top5_err)
            self.mb_target_top1_err.add_value(target_top1_err)
            self.mb_target_top5_err.add_value(target_top5_err)
            # Aggregate stats
            self.num_top1_mis += top1_err * mb_size
            self.num_top5_mis += top5_err * mb_size
            self.num_target_top1_mis += top1_err * mb_size
            self.num_target_top5_mis += top5_err * mb_size
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            stats["top1_err"] = self.mb_top1_err.get_win_median()
            stats["top5_err"] = self.mb_top5_err.get_win_median()
            stats["target_top1_err"] = self.mb_target_top1_err.get_win_median()
            stats["target_top5_err"] = self.mb_target_top5_err.get_win_median()
        logging.log_json_stats(stats)
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            top1_err = self.num_top1_mis / self.num_samples
            top5_err = self.num_top5_mis / self.num_samples
            target_top1_err = self.num_target_top1_mis / self.num_samples
            target_top5_err = self.num_target_top5_mis / self.num_samples

            avg_loss = self.loss_total / self.num_samples
            stats["top1_err"] = top1_err
            stats["top5_err"] = top5_err
            stats["target_top1_err"] = target_top1_err
            stats["target_top5_err"] = target_top5_err
            stats["loss"] = avg_loss
        logging.log_json_stats(stats)
